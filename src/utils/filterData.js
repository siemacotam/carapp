export const filterData = (
  ...
  if (filterOption.AVAILABLE) {
    if (filterOption.FULL) {
      data.push(
        carsData.filter(
          (i) => i.status === "AVAILABLE" && i.batteryLevelPct > 75
        )
      );
    }
    if (filterOption.HALF) {
      data.push(
        carsData.filter(
          (i) => i.status === "AVAILABLE" && i.batteryLevelPct > 50
        )
      );
    }
    if (!filterOption.HALF && !filterOption.FULL) {
      data.push(carsData.filter((i) => i.status === "AVAILABLE"));
    }
  }
  if (filterOption.TAKEN) {
    if (filterOption.FULL) {
      data.push(
        carsData.filter((i) => i.status === "TAKEN" && i.batteryLevelPct > 75)
      );
    }
    if (filterOption.HALF) {
      data.push(
        carsData.filter((i) => i.status === "TAKEN" && i.batteryLevelPct > 50)
      );
    }
    if (!filterOption.HALF && !filterOption.FULL) {
